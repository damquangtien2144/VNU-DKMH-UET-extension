...
async function loadData() {
    const result = await chrome.storage.local.get(['subjects', 'labels', 'isPinned', 'highlightSettings', 'onboardingDone']);
...
function saveData() {
    chrome.storage.local.set({ subjects, labels, highlightSettings });
}
...
if (isIframeMode) {
    ...
    btnTogglePin.addEventListener('click', async () => {
        ...
        try {
            await chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ['scripts/content.js'] });
            chrome.tabs.sendMessage(tab.id, { action: "INJECT_PANEL" });
